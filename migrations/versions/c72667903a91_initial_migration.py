...
def upgrade():
    ...
    # This appears to be an initial migration that assumes existing tables
    # Skip if this is a fresh database
    ...
    # Check if we have any of the tables that would be dropped
    existing_tables = inspector.get_table_names()
    if 'kanban_board' not in existing_tables and 'company_settings' not in existing_tables:
        # This is likely a fresh database, skip this migration
        return
    # Drop tables that might not exist
    for table_name in ['kanban_card', 'kanban_column', 'kanban_board']:
        try:
            op.drop_table(table_name)
        except:
            pass  # Table might not exist
    
    # Drop invitation indexes that might not exist
    for idx_name in ['idx_invitation_company', 'idx_invitation_email', 'idx_invitation_expires', 'idx_invitation_token']:
        try:
            op.drop_index(op.f(idx_name), table_name='company_invitation')
        except:
            pass  # Index might not exist
    
    # Drop company settings constraint and columns
    try:
    ...
    for column_name in ['allow_team_visibility_comments', 'round_time_to', 'time_tracking_mode', 
                        'enable_projects', 'default_comment_visibility', 'require_task_assignment',
                        'enable_teams', 'require_project_selection', 'restrict_project_access_by_team',
                        'enable_invoicing', 'auto_break_deduction', 'allow_task_creation_by_members',
                        'created_by_id', 'enable_reports', 'require_daily_notes', 'min_hours_per_entry',
                        'default_currency', 'allow_manual_time']:
        try:
            op.drop_column('company_settings', column_name)
        except:
    ...
    # Drop note-related indexes that might not exist
    for idx_name, table_name in [
        ('idx_note_folder', 'note'),
        ('idx_note_folder_company', 'note_folder'),
        ('idx_note_folder_created_by', 'note_folder'),
        ('idx_note_folder_parent_path', 'note_folder')
    ]:
        try:
            op.drop_index(op.f(idx_name), table_name=table_name)
        except:
            pass  # Index might not exist
    
    # Drop and recreate note_link constraints
    try:
    ...
    try:
        ...
        pass  # Foreign keys might already exist or table might not exist
    
    # Drop note_share indexes that might not exist
    for idx_name in ['idx_note_share_created_by', 'idx_note_share_note_id', 'idx_note_share_token']:
        try:
            op.drop_index(op.f(idx_name), table_name='note_share')
        except:
            pass  # Index might not exist
    ...
    # Alter project_category columns
    try:
    ...
    for column_name in ['icon', 'created_by_id', 'updated_at']:
        try:
            op.drop_column('project_category', column_name)
        except:
            pass
    
    # Alter task status columns
    try:
        ...
        pass  # Tables or columns might not exist
    ...
    # Drop user_preferences columns that might not exist
    for column_name in ['daily_summary', 'mention_notifications', 'pomodoro_duration',
                        'keyboard_shortcuts', 'week_start', 'default_project_id',
                        'sound_enabled', 'email_notifications', 'task_assigned_notifications',
                        'pomodoro_enabled', 'email_daily_summary', 'email_weekly_summary',
                        'task_reminders', 'auto_start_timer', 'weekly_report',
                        'push_notifications', 'compact_mode', 'pomodoro_break',
                        'idle_time_detection', 'task_completed_notifications', 'show_weekends']:
        try:
            op.drop_column('user_preferences', column_name)
        except:
            pass  # Column might not exist
